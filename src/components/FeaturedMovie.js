...
export default ({item}) => {
    ...
    let description = item.overview   // Para manipular a descrição do filme em featured
    if (description.length > 200) {   // Se o texto da descrição for maior que 20 caracteres
        description = description.substring(0, 200)+'...';  //Aí vai de 0 à 200, concatenado com reticiências
    } 
            ...
            <section className="featured" style={{  // Configurando  estilo da imagem do Filme em Destaque
                ...
                <div className="featured--vertical">    {/* Divisão correspondente à parte vertical da imagem do filme em destaque */}
                    <div className="featured--horizontal">    {/* Divisão correspondente à parte horizontal da imagem do filme em destaque */}
                        ...
                        <div className="featured--description">{description}</div>    {/* Correspondente à descrição, sendo que o "overview" é referente a informação da descrição do filme dada na API da tmdb */}
